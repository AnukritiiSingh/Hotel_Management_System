/**
 *
 * @author anukriti singh
 */
package hotel.management.system;

import javax.swing.*;
import java.awt.*;
import java.awt.event.*;

public class AddEmployee extends JFrame implements ActionListener{
    JTextField t1,t2,t3,t4,t5,t6;
    JRadioButton r1,r2;
    JComboBox c1;
    JButton b1;
    AddEmployee(){
    JLabel name = new JLabel("NAME");
    name.setFont(new Font("Tahoma",Font.BOLD, 18));
    name.setBounds(60,20,120,30);
     add(name);
    t1 =new JTextField();
    t1.setBounds(210,20,200,30);
    add(t1);
    
     JLabel age = new JLabel("AGE");
    age.setFont(new Font("Tahoma",Font.BOLD,18 ));
    age.setBounds(60,70,120,30);
     add(age);
    t2 =new JTextField();
    t2.setBounds(210,70,200,30);
    add(t2);
    
     JLabel gender = new JLabel("GENDER");
    gender.setFont(new Font("Tahoma",Font.BOLD, 19));
    gender.setBounds(60,120,120,30);
  
    add(gender);
    r1 = new JRadioButton("Male");
    r1.setFont(new Font("Tahoma",Font.BOLD, 19));
    r1.setBounds(210,120,120,30);
    r1.setBackground(Color.WHITE);
    add(r1);
    
     r2 = new JRadioButton("Female");
    r2.setFont(new Font("Tahoma",Font.BOLD, 19));
    r2.setBounds(330,120,120,30);
    r2.setBackground(Color.WHITE);
    add(r2);
    
    JLabel job = new JLabel("JOB");
    job.setFont(new Font("Tahoma",Font.BOLD, 19));
    job.setBounds(60,180,120,30);
    add(job);
  
    String str[] = {"Front Desk Clerks","Housekeeping","Porters","Kitchen Staff","Room Service","Waiter/Waitress","Manager","Accountant","Chef","Cleaning"};
       
    c1 = new JComboBox(str);
    c1.setBackground(Color.WHITE);
    c1.setFont(new Font("Tahoma", Font.BOLD, 14));
    add(c1);
    c1.setBounds(210,180,200,30);
    JLabel salary = new JLabel("SALARY");
    salary.setFont(new Font("Tahoma",Font.BOLD, 19));
    salary.setBounds(60,240,120,30);
    add(salary);
    t3=new JTextField();
    t3.setBounds(210,240,200,30);
    add(t3);
    
    JLabel phone  = new JLabel("PHONE");
    phone.setFont(new Font("Tahoma",Font.BOLD, 19));
    phone.setBounds(60,290,120,30);
    add(phone);
    t4=new JTextField();
    t4.setBounds(210,290,200,30);
    add(t4);
    
    JLabel aadhar = new JLabel("AADHAR");
    aadhar.setFont(new Font("Tahoma",Font.BOLD, 19));
    aadhar.setBounds(60,340,120,30);
    add(aadhar);
    t5=new JTextField();
    t5.setBounds(210,340,200,30);
    add(t5);
    JLabel email= new JLabel("EMAIL");
    email.setFont(new Font("Tahoma",Font.BOLD, 19));
    email.setBounds(60,390,120,30);
    add(email);
    
    t6=new JTextField();
    t6.setBounds(210,390,200,30);
    add(t6);
    b1 = new JButton("Submit");
    b1.setBackground(Color.BLACK);
    b1.setForeground(Color.WHITE);
    b1.setBounds(210,440,150,35);
    b1.addActionListener(this);
    add(b1);
    
    ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("hotel/management/system/icons/fourth.jpg"));
    Image i2= i1.getImage().getScaledInstance(600,500,Image.SCALE_DEFAULT);
    ImageIcon i3 = new ImageIcon(i2);
    JLabel l1 = new JLabel(i3);
    l1.setBounds(400,0,600,630);
    add(l1);
    
    JLabel l2 = new JLabel("ADD EMPLOYEE DETAILS");
       l2.setForeground(Color.BLUE);
l2.setBounds(450,30,600,30);
    l2.setFont(new Font("Tahoma",Font.BOLD,30));
    add(l2);
    
    getContentPane().setBackground(Color.WHITE);
    
    setLayout(null);
    setBounds(225,150,930,530);
    setVisible(true);
    
            
    }
    public void actionPerformed(ActionEvent ae){
        String name = t1.getText();
        String age = t2.getText();
        String salary = t3.getText();
        String phone= t4.getText();
        String aadhar = t5.getText();
        String email = t6.getText();
        
        String gender = null;
        if(r1.isSelected()){
            gender = "male";
        }else if(r2.isSelected()){
            gender = "Female";
        }
        
        String job = (String)c1.getSelectedItem();
        conn c = new conn();
        String str = "insert into employee values('"+name+"','"+age+"','"+gender+"','"+job+"','"+salary+"','"+phone+"','"+aadhar+"','"+email+"')";
         try{
           c.s.executeUpdate(str);
           JOptionPane.showMessageDialog(null,"Now Employee Added");
           this.setVisible(false);
        
         }catch(Exception e){
             System.out.println(e);
         }
    
    }
    public static void main(String[] args){
        new AddEmployee().setVisible(true);
        
   }
}
